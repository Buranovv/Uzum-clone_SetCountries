...
export function renderFn(array, parent) {
  parent.innerHTML = "";
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    newCard.innerHTML = `
        <div class="img-box">
           <img class="card-img-top" width="110"  src="${
             array[i].image
           }" alt="">
           <svg
      width="20"
    ...
    >
      ...
      data-id="${array[i].id}"
        ...
        fill="${array[i].isFav ? "#8967F0" : "white"}"
        fill-opacity="0.8"
      ></path>
      <path
      id="ui-icon"
      data-id="${array[i].id}"
       class="ui-icon-hover"
        ...
        fill="${array[i].isFav ? "#8967F0" : "#15151A60"}"
      ></path>
    </svg>  
        </div>
        <div class="card-body">
          ...
          <div class="btn-box d-flex align-items-center justify-content-center gap-2">
          <button data-id="${
            array[i].id
          }" class="btn btn-danger w-100 d-none" id="delete-btn">Delete</button>
          <button data-id="${
            array[i].id
          }" class="btn btn-primary w-100 d-none" id="edit-btn">Edit</button>
          </div>
          </div>
        </div>
        `;
